...
func Test_XPathSelectRelativeStart(t *testing.T) {
	...
	tests := []navigator.XpathSelect{
		...
		//	XPath: "/switch-model[@switch-model-id=$this/model-id]/display-name",
		//	Expected: []string{
		//		"Iter Value: display-name: Super Switch 1610",
		//	},
		//},
...
// Test_XPathSelectRelativePath - start each test from switch[1]/port[2]/channel-number
func Test_XPathSelectRelativePathChannelNumber(t *testing.T) {
	sampleConfig, err := ioutil.ReadFile("../examples/full-config-example-1.json")
	if err != nil {
		assert.NoError(t, err)
	}
	...
	tests := []navigator.XpathSelect{
		{
			...
			XPath: "../display-name",
			Expected: []string{
				"Iter Value: display-name: Port 2/0 on switch 1",
			},
		},
		{
			Name:  "this (channel-number)",
			XPath: "$this",
			Expected: []string{
				"Iter Value: channel-number: 2",
			},
		},
		{
			Name:  "available cage-numbers on corresponding model",
			XPath: "/switch-model[@switch-model-id=$this/../../model-id]/port/@cage-number",
			Expected: []string{
				"Iter Value: cage-number: 1",
				"Iter Value: cage-number: 2",
				"Iter Value: cage-number: 3",
				"Iter Value: cage-number: 4",
			},
		},
		{
			Name:  "available cage-numbers on corresponding model but not the one current",
			XPath: "/switch-model[@switch-model-id=$this/../../model-id]/port[@cage-number!=$this]/@cage-number",
			Expected: []string{
				"Iter Value: cage-number: 1",
				"Iter Value: cage-number: 3",
				"Iter Value: cage-number: 4",
		...
		{
			Name:  "allowable channel-numbers on from corresponding switch-model port",
			XPath: "/switch-model[@switch-model-id=$this/../../model-id]/port[@cage-number=$this/../@cage-number]/max-channel",
			Expected: []string{
				"Iter Value: max-channel: 2",
			},
		},
	}
