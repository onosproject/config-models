...
func (c *GnmiClient) UpdateItemCont1AList2A(ctx context.Context, target string, data OnfTest1_Cont1A_List2A,
...
func (c *GnmiClient) UpdateCont1AList2A(ctx context.Context, target string, list map[string]*OnfTest1_Cont1A_List2A,
) (*gnmi.SetResponse, error) {
	gnmiCtx, cancel := context.WithTimeout(ctx, 10*time.Second)
	defer cancel()

	pathEls := []*gnmi.PathElem{
		{
			Name: "cont1a",
		},
		{
	...
	for _, item := range list {
		...
		itemPath := &gnmi.Path{
			Elem:   append(pathEls, &gnmi.PathElem{Name: *item.Name}),
			Target: target,
		}

		// TODO if it's pointer, pass the value
		// if it's a value pass it directly
		r, err := gnmi_utils.CreateGnmiSetForContainer(ctx, reflect.Indirect(reflect.ValueOf(item)), itemPath, target)
		if err != nil {
			return nil, err
		}
...
func (c *GnmiClient) GetCont1AList2A(ctx context.Context, target string, key string,
