			...
			Elem: []*gnmi.PathElem{
				...
				},
				{
					...
					Key: map[string]string{
						...
						"index1": fmt.Sprint(key.Index1),

						"index2": fmt.Sprint(key.Index2),
					},
				},
			},
			Target: target,
	...
	path := []*gnmi.Path{
		{
			Elem: []*gnmi.PathElem{
				...
				},
				{
					Name: "list2b",
					Key: map[string]string{
						"index1": fmt.Sprint(*data.Index1),
						"index2": fmt.Sprint(*data.Index2),
					},
				},
			},
			Target: target,
...
func (c *GnmiClient) Update_Cont1BState_List2B_List(ctx context.Context, target string, list map[OnfTest1_Cont1BState_List2B_Key]*OnfTest1_Cont1BState_List2B,
) (*gnmi.SetResponse, error) {
	gnmiCtx, cancel := context.WithTimeout(ctx, 10*time.Second)
	defer cancel()
	...
	for _, item := range list {
		...
		path := &gnmi.Path{
			Elem: append(basePathElems, &gnmi.PathElem{
				Name: "list2a",
				Key: map[string]string{
					"index1": fmt.Sprint(*item.Index1),
					"index2": fmt.Sprint(*item.Index2),
				},
			}),
			Target: target,
		}
