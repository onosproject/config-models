...
// A plugin for the YGOT model of aether-2.2.0.
package modelplugin
...
import (
	...
	"github.com/onosproject/config-models/modelplugin/aether-2.2.0/aether_2_2_0"
...
const modulename = "aether.so.2.2.0"
...
func (m Modelplugin) UnmarshalConfigValues(jsonTree []byte) (*ygot.ValidatedGoStruct, error) {
	device := &aether_2_2_0.Device{}
	vgs := ygot.ValidatedGoStruct(device)

	if err := aether_2_2_0.Unmarshal([]byte(jsonTree), device); err != nil {
		return nil, err
	}
...
func (m Modelplugin) Validate(ygotModel *ygot.ValidatedGoStruct, opts ...ygot.ValidationOption) error {
	...
	device, ok := deviceDeref.(*aether_2_2_0.Device)
	if !ok {
		return fmt.Errorf("unable to convert model in to aether_2_2_0")
	}
	return device.Validate()
}

func (m Modelplugin) Schema() (map[string]*yang.Entry, error) {
	return aether_2_2_0.UnzipSchema()
}
