...
func Test_GetPathValues(t *testing.T) {
	...
	for _, pathValue := range pathValues {
		...
		case "/cont1a/list2a[name=1]/tx-power":
			...
			assert.Equal(t, configapi.ValueType_UINT, (&value).Type)
		case "/cont1a/list2a[name=1]/range-min":
			assert.Equal(t, "2", (&value).ValueToString())
			assert.Equal(t, configapi.ValueType_UINT, (&value).Type)
		case "/cont1a/list2a[name=1]/range-max":
			assert.Equal(t, "4", (&value).ValueToString())
			assert.Equal(t, configapi.ValueType_UINT, (&value).Type)
		case "/cont1a/list5[key1=0][key2=*]/key1":
			assert.Equal(t, "five", (&value).ValueToString())
			assert.Equal(t, configapi.ValueType_STRING, (&value).Type)
		case "/cont1a/list5[key1=0][key2=*]/key2":
			assert.Equal(t, "6", (&value).ValueToString())
			assert.Equal(t, configapi.ValueType_UINT, (&value).Type)
		case "/cont1a/list5[key1=0][key2=*]/leaf5a":
			assert.Equal(t, "5a five-6", (&value).ValueToString())
			assert.Equal(t, configapi.ValueType_STRING, (&value).Type)
		case "/cont1a/list5[key1=1][key2=*]/key1":
			assert.Equal(t, "five", (&value).ValueToString())
			assert.Equal(t, configapi.ValueType_STRING, (&value).Type)
		case "/cont1a/list5[key1=1][key2=*]/key2":
			assert.Equal(t, "7", (&value).ValueToString())
			assert.Equal(t, configapi.ValueType_UINT, (&value).Type)
		case "/cont1a/list5[key1=1][key2=*]/leaf5a":
			assert.Equal(t, "5a five-7", (&value).ValueToString())
			assert.Equal(t, configapi.ValueType_STRING, (&value).Type)
		case "/cont1a/list4[id=0]/id":
			assert.Equal(t, "l2a1", (&value).ValueToString())
			assert.Equal(t, configapi.ValueType_STRING, (&value).Type)
		case "/cont1a/list4[id=0]/leaf4b":
			assert.Equal(t, "this is list4-l2a1", (&value).ValueToString())
			assert.Equal(t, configapi.ValueType_STRING, (&value).Type)
		case "/cont1a/list4[id=0]/list4a[fkey1=0][fkey2=*]/fkey1":
			assert.Equal(t, "five", (&value).ValueToString())
			assert.Equal(t, configapi.ValueType_STRING, (&value).Type)
		case "/cont1a/list4[id=0]/list4a[fkey1=0][fkey2=*]/fkey2":
			assert.Equal(t, "7", (&value).ValueToString()) // TODO should be UINT
			assert.Equal(t, configapi.ValueType_STRING, (&value).Type)
		case "/cont1a/list4[id=0]/list4a[fkey1=0][fkey2=*]/displayname":
			assert.Equal(t, "Value l2a1-five-7", (&value).ValueToString())
			assert.Equal(t, configapi.ValueType_STRING, (&value).Type)
		case "/cont1a/list4[id=0]/list4a[fkey1=1][fkey2=*]/fkey1":
			assert.Equal(t, "five", (&value).ValueToString())
			assert.Equal(t, configapi.ValueType_STRING, (&value).Type)
		case "/cont1a/list4[id=0]/list4a[fkey1=1][fkey2=*]/fkey2":
			assert.Equal(t, "6", (&value).ValueToString())
			assert.Equal(t, configapi.ValueType_STRING, (&value).Type) // TODO should be UINT
		case "/cont1a/list4[id=0]/list4a[fkey1=1][fkey2=*]/displayname":
			assert.Equal(t, "Value l2a1-five-6", (&value).ValueToString())
			assert.Equal(t, configapi.ValueType_STRING, (&value).Type)
		case "/cont1a/list4[id=0]/list4a[fkey1=2][fkey2=*]/fkey1":
			assert.Equal(t, "six", (&value).ValueToString())
			assert.Equal(t, configapi.ValueType_STRING, (&value).Type)
		case "/cont1a/list4[id=0]/list4a[fkey1=2][fkey2=*]/fkey2":
			assert.Equal(t, "6", (&value).ValueToString())
			assert.Equal(t, configapi.ValueType_STRING, (&value).Type) // TODO should be UINT
		case "/cont1a/list4[id=0]/list4a[fkey1=2][fkey2=*]/displayname":
			assert.Equal(t, "Value l2a1-six-6", (&value).ValueToString())
			assert.Equal(t, configapi.ValueType_STRING, (&value).Type)
		case "/cont1a/list4[id=1]/id":
			assert.Equal(t, "l2a2", (&value).ValueToString())
			assert.Equal(t, configapi.ValueType_STRING, (&value).Type)
		case "/cont1a/list4[id=1]/leaf4b":
...
func Test_findModelRwPathNoIndicesNew(t *testing.T) {
	tests := map[string]*findIdxTestRwTest{
		`/cont1a/leaf1a`: {
			pathObjStr:  `path:"/cont1a/leaf1a" value_type:STRING description:"Leaf inside Container 1a" length:"5..10" AttrName:"leaf1a" `,
			pathWithIdx: `/cont1a/leaf1a`,
			found:       true,
		},
		`/leafAtTopLevel`: {
			pathObjStr:  `path:"/leafAtTopLevel" value_type:STRING description:"A leaf at the top level (not recommended but must be supported)" AttrName:"leafAtTopLevel" `,
			pathWithIdx: `/leafAtTopLevel`,
			found:       true,
		},
		`//leafAtTopLevel`: { // With double slash
			pathObjStr:  `path:"/leafAtTopLevel" value_type:STRING description:"A leaf at the top level (not recommended but must be supported)" AttrName:"leafAtTopLevel" `,
			pathWithIdx: `/leafAtTopLevel`,
			found:       true,
		},
		`/cont1a/list4[test1]/leaf4b`: {
			pathObjStr:  `path:"/cont1a/list4[id=*]/leaf4b" value_type:STRING description:"leaf 4a on list4a elements" length:"1..20" AttrName:"leaf4b" `,
			pathWithIdx: `/cont1a/list4[id=test1]/leaf4b`,
			found:       true,
		},
		// TODO fix this - does not handle double indices properly
		//`/cont1a/list4[test1]/list4a[k1][k2]/displayname`: {
		//	pathObjStr: `path:"/cont1a/list4[id=*]/list4a[fkey1=*][fkey2=*]/displayname" value_type:STRING description:"an optional display name attribute with 2 different length ranges" length:"1..5" length:"10..20" AttrName:"displayname" `,
		//	pathWithIdx:  `/cont1a/list4[id=test1]/list4a[fkey1=k1][fkey2=k2]/displayname`,
		//	found:      true,
		//},
	}
...
func Test_findModelRoPathNoIndicesNew(t *testing.T) {
	tests := map[string]*findIdxTestRwTest{
		...
		`/cont1b-state/list2b[5]/leaf3c`: {
			...
			pathWithIdx: `/cont1b-state/list2b[index=5]/leaf3c`,
			found:       true,
		},
	}
...
func Test_handleAttribute(t *testing.T) {
	...
	tests := map[string]handleAttrTest{
		`/cont1a/leaf1a`: {
			value:         `test-string`,
			expectedPath:  `/cont1a/leaf1a`,
			expectedValue: `test-string`,
			expectedType:  configapi.ValueType_STRING,
		},
		`/cont1a/cont2a/leaf2a`: {
			value:         `1`,
			expectedPath:  `/cont1a/cont2a/leaf2a`,
			expectedValue: `1`,
			expectedType:  configapi.ValueType_UINT,
		},
		`/cont1a/list2a[2a-1]/tx-power`: {
			value:         `6`,
			expectedPath:  `/cont1a/list2a[name=2a-1]/tx-power`,
			expectedValue: `6`,
			expectedType:  configapi.ValueType_UINT,
		},
		`/cont1b-state/list2b[5]/leaf3c`: {
			value:         `test-string`,
			expectedPath:  `/cont1b-state/list2b[index=5]/leaf3c`,
			expectedValue: `test-string`,
			expectedType:  configapi.ValueType_STRING,
		},
		`/cont1a/leaf-non-existent`: {
			errString: `unable to locate /cont1a/leaf-non-existent in model`,
		},
	}
